...
#
# A previewer extracts a preview image from a blob. Pro Active Storage provides previewers for videos and PDFs:
# ProActiveStorage::Previewer::VideoPreviewer and ProActiveStorage::Previewer::PDFPreviewer. Build custom previewers by
# subclassing ProActiveStorage::Previewer and implementing the requisite methods. Consult the ProActiveStorage::Previewer
# documentation for more details on what's required of previewers.
#
# To choose the previewer for a blob, Pro Active Storage calls +accept?+ on each registered previewer in order. It uses the
# first previewer for which +accept?+ returns true when given the blob. In a Rails application, add or remove previewers
# by manipulating +Rails.application.config.pro_active_storage.previewers+ in an initializer:
#
#   Rails.application.config.pro_active_storage.previewers
#   # => [ ProActiveStorage::Previewer::PDFPreviewer, ProActiveStorage::Previewer::VideoPreviewer ]
#
#   # Add a custom previewer for Microsoft Office documents:
#   Rails.application.config.pro_active_storage.previewers << DOCXPreviewer
#   # => [ ProActiveStorage::Previewer::PDFPreviewer, ProActiveStorage::Previewer::VideoPreviewer, DOCXPreviewer ]
#
# Outside of a Rails application, modify +ProActiveStorage.previewers+ instead.
#
...
class ProActiveStorage::Preview
  class UnprocessedError < StandardError; end

  attr_reader :blob, :variation

  def initialize(blob, variation_or_variation_key)
    @blob, @variation = blob, ProActiveStorage::Variation.wrap(variation_or_variation_key)
  end
  ...
  # Returns the URL of the preview's variant on the service. Raises ProActiveStorage::Preview::UnprocessedError if the
  ...
  private
    ...
    def variant
      ProActiveStorage::Variant.new(image, variation).processed
    end
    ...
    def previewer_class
      ProActiveStorage.previewers.detect { |klass| klass.accept?(blob) }
    end
end
