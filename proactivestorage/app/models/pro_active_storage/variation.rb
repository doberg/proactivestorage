...
# the ProActiveStorage::Blob#variant method and should rarely be used directly.
#
# In case you do need to use this directly, it's instantiated using a hash of transformations where
# the key is the command and the value is the arguments. Example:
#
#   ProActiveStorage::Variation.new(resize_to_fit: [100, 100], monochrome: true, trim: true, rotate: "-90")
#
# The options map directly to {ImageProcessing}[https://github.com/janko-m/image_processing] commands.
class ProActiveStorage::Variation
  attr_reader :transformations

  class << self
    # Returns a Variation instance based on the given variator. If the variator is a Variation, it is
    # returned unmodified. If it is a String, it is passed to ProActiveStorage::Variation.decode. Otherwise,
    ...
    def decode(key)
      new ProActiveStorage.verifier.verify(key, purpose: :variation)
    end

    # Returns a signed key for the +transformations+, which can be used to refer to a specific
    # variation in a URL or combined key (like <tt>ProActiveStorage::Variant#key</tt>).
    def encode(transformations)
      ProActiveStorage.verifier.generate(transformations, purpose: :variation)
    end
  end
  ...
  def transform(file, format: nil)
    ActiveSupport::Notifications.instrument("transform.pro_active_storage") do
      if processor
  ...
  private
    ...
    def image_processing_transform(file, format)
      operations = transformations.each_with_object([]) do |(name, argument), list|
        if name.to_s == "combine_options"
          ActiveSupport::Deprecation.warn("The ImageProcessing ProActiveStorage variant backend doesn't need :combine_options, as it already generates a single MiniMagick command. In Rails 6.1 :combine_options will not be supported anymore.")
    ...
    # Returns the ImageProcessing processor class specified by `ProActiveStorage.variant_processor`.
    def processor
      begin
      ...
      ImageProcessing.const_get(ProActiveStorage.variant_processor.to_s.camelize) if ProActiveStorage.variant_processor
    end
