...
class ProActiveStorage::ReflectionTest < ActiveSupport::TestCase
  test "allows reflecting for all attachment" do
    expected_classes =
      User.reflect_on_all_attachments.all? do |reflection|
        reflection.is_a?(ProActiveStorage::Reflection::HasOneAttachedReflection) ||
          reflection.is_a?(ProActiveStorage::Reflection::HasManyAttachedReflection)
      end
