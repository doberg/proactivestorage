...
class ProActiveStorage::BlobTest < ActiveSupport::TestCase
  ...
  test ".unattached scope returns not attached blobs" do
    ...
    ProActiveStorage::Blob.delete_all
    ...
    assert_equal [blob_2], ProActiveStorage::Blob.unattached
    user.destroy
    assert_equal [blob_1, blob_2].map(&:id).sort, ProActiveStorage::Blob.unattached.pluck(:id).sort
  end

  test "blob generates key according to custom prefix in has_one_attached" do
    class UserWithHasOneAttachedCustomKeyFormat < User
      has_one_attached :avatar, prefix: ":environment/:namespace/:hash/:filename.:extension"
    end
  ...
  test "blob generates key according to custom prefix in has_many_attached" do
    class UserWithHasManyAttachedCustomKeyFormat < User
      has_many_attached :avatars, prefix: ":environment/:namespace/:hash/:filename.:extension"
    end
  ...
  test "blob saves prefix used to generate key" do
    class UserWithHasOneAttachedCustomKeyFormat < User
      has_one_attached :avatar, prefix: ":environment/:namespace/:hash/:filename.:extension"
    end
    ...
    assert_match(":environment/:namespace/:hash/:filename.:extension", user.avatar.prefix)
  end

  test "blob by default returns prefix defined in configuration" do
    blob = ProActiveStorage::Blob.new
    default_prefix = ProActiveStorage.default_prefix

    assert_equal blob.prefix, default_prefix
  end
  ...
  test "open with integrity" do
    create_file_blob(filename: "racecar.jpg").tap do |blob|
      blob.open do |file|
        ...
        assert File.basename(file.path).starts_with?("ProActiveStorage-#{blob.id}-")
  ...
  test "open without integrity" do
    create_blob(data: "Hello, world!").tap do |blob|
      ...
      assert_raises ProActiveStorage::IntegrityError do
  ...
  test "urls allow for custom filename" do
    ...
    new_filename = ProActiveStorage::Filename.new("new.txt")
  ...
  test "purge deletes file from external service" do
    ...
    blob.purge
    assert_not ProActiveStorage::Blob.service.exist?(blob.key)
  end
  ...
  test "purge deletes variants from external service" do
    ...
    blob.purge
    assert_not ProActiveStorage::Blob.service.exist?(variant.key)
  end
  ...
  private
    def expected_url_for(blob, disposition: :inline, filename: nil)
      ...
      "https://example.com/rails/pro_active_storage/disk/#{ProActiveStorage.verifier.generate(blob.key, expires_in: 5.minutes, purpose: :blob_key)}/#{filename}?#{query_string}"
    end
end
