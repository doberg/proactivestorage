...
require "tmpdir"
ProActiveStorage::Blob.service = ProActiveStorage::Service::DiskService.new(root: Dir.mktmpdir("pro_active_storage_tests"))

ProActiveStorage.logger = ActiveSupport::Logger.new(nil)
ProActiveStorage.verifier = ActiveSupport::MessageVerifier.new("Testing")

class ActiveSupport::TestCase
  self.file_fixture_path = File.expand_path("fixtures/files", __dir__)

  setup do
    ProActiveStorage::Current.host = "https://example.com"
  end

  teardown do
    ProActiveStorage::Current.reset
  end

  private
    def create_blob(data: "Hello world!", filename: "hello.txt", content_type: "text/plain", identify: true)
      ProActiveStorage::Blob.create_after_upload! io: StringIO.new(data), filename: filename, content_type: content_type, identify: identify
    end

    def create_file_blob(filename: "racecar.jpg", content_type: "image/jpeg", metadata: nil)
      ProActiveStorage::Blob.create_after_upload! io: file_fixture(filename).open, filename: filename, content_type: content_type, metadata: metadata
    end

    def create_blob_before_direct_upload(filename: "hello.txt", byte_size:, checksum:, content_type: "text/plain")
      ProActiveStorage::Blob.create_before_direct_upload! filename: filename, byte_size: byte_size, checksum: checksum, content_type: content_type
    end
    ...
    def directly_upload_file_blob(filename: "racecar.jpg", content_type: "image/jpeg")
      ...
      create_blob_before_direct_upload(filename: filename, byte_size: byte_size, checksum: checksum, content_type: content_type).tap do |blob|
        ProActiveStorage::Blob.service.upload(blob.key, file.open)
      end
    end
...
require "global_id"
GlobalID.app = "ProActiveStorageExampleApp"
