...
module SharedGeneratorTests
  ...
  def test_default_frameworks_are_required_when_others_are_removed
    ...
    assert_file "#{application_path}/config/application.rb", /^# require\s+["']pro_active_storage\/engine["']/
  ...
  def test_generator_for_pro_active_storage
    run_generator

    assert_file "#{application_path}/app/assets/javascripts/application.js" do |content|
      assert_match(/^\/\/= require proactivestorage/, content)
    end

    assert_file "#{application_path}/config/environments/development.rb" do |content|
      assert_match(/config\.pro_active_storage/, content)
    end
  ...
  def test_generator_if_skip_pro_active_storage_is_given
    run_generator [destination_root, "--skip-pro-active-storage"]

    assert_file "#{application_path}/config/application.rb", /#\s+require\s+["']pro_active_storage\/engine["']/

    assert_file "#{application_path}/app/assets/javascripts/application.js" do |content|
      assert_no_match(/^\/\/= require proactivestorage/, content)
    end

    assert_file "#{application_path}/config/environments/development.rb" do |content|
      assert_no_match(/config\.pro_active_storage/, content)
    end
  ...
  def test_generator_does_not_generate_pro_active_storage_contents_if_skip_active_record_is_given
    run_generator [destination_root, "--skip-active-record"]
