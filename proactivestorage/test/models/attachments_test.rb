...
class ProActiveStorage::AttachmentsTest < ActiveSupport::TestCase
  ...
  test "replace attached blob" do
    ...
    perform_enqueued_jobs do
      assert_no_difference -> { ProActiveStorage::Blob.count } do
  ...
  test "replace attached blob unsuccessfully" do
    ...
    assert ProActiveStorage::Blob.service.exist?(@user.avatar.key)
  end
  ...
  test "replace independent attached blob" do
    ...
    perform_enqueued_jobs do
      assert_difference -> { ProActiveStorage::Blob.count }, +1 do
        assert_no_difference -> { ProActiveStorage::Attachment.count } do
  ...
  test "attach blob to new record" do
    ...
    assert_difference -> { ProActiveStorage::Attachment.count }, +1 do
      user.save!
    end
  ...
  test "detach blob" do
    ...
    assert ProActiveStorage::Blob.exists?(avatar_blob_id)
    assert ProActiveStorage::Blob.service.exist?(avatar_key)
  end
  ...
  test "purge attached blob" do
    ...
    @user.avatar.purge
    assert_not_predicate @user.avatar, :attached?
    assert_not ProActiveStorage::Blob.service.exist?(avatar_key)
  end
  ...
  test "purge attached blob later when the record is destroyed" do
    ...
    perform_enqueued_jobs do
      ...
      assert_nil ProActiveStorage::Blob.find_by(key: avatar_key)
      assert_not ProActiveStorage::Blob.service.exist?(avatar_key)
    end
  end
  ...
  test "delete attachment for independent blob when record is destroyed" do
    ...
    @user.destroy
    assert_not ProActiveStorage::Attachment.exists?(record: @user, name: "cover_photo")
  end
  ...
  test "attach blobs to new record" do
    ...
    assert_difference -> { ProActiveStorage::Attachment.count }, +2 do
      user.save!
    end
  ...
  test "detach blobs" do
    ...
    assert ProActiveStorage::Blob.exists?(highlight_blob_ids.first)
    assert ProActiveStorage::Blob.exists?(highlight_blob_ids.second)

    assert ProActiveStorage::Blob.service.exist?(highlight_keys.first)
    assert ProActiveStorage::Blob.service.exist?(highlight_keys.second)
  end
  ...
  test "purge attached blobs" do
    ...
    assert_not_predicate @user.highlights, :attached?
    assert_not ProActiveStorage::Blob.service.exist?(highlight_keys.first)
    assert_not ProActiveStorage::Blob.service.exist?(highlight_keys.second)
  end
  ...
  test "purge attached blobs later when the record is destroyed" do
    ...
    perform_enqueued_jobs do
      ...
      assert_nil ProActiveStorage::Blob.find_by(key: highlight_keys.first)
      assert_not ProActiveStorage::Blob.service.exist?(highlight_keys.first)

      assert_nil ProActiveStorage::Blob.find_by(key: highlight_keys.second)
      assert_not ProActiveStorage::Blob.service.exist?(highlight_keys.second)
    end
  end
  ...
  test "delete attachments for independent blobs when the record is destroyed" do
    ...
    @user.destroy
    assert_not ProActiveStorage::Attachment.exists?(record: @user, name: "vlogs")
  end
  ...
  test "selectively purge one attached blob of many" do
    ...
    assert_not ProActiveStorage::Blob.exists?(key: first_blob.key)
    assert ProActiveStorage::Blob.exists?(key: second_blob.key)
  end
