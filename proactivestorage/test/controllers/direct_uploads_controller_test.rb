...
if SERVICE_CONFIGURATIONS[:s3] && SERVICE_CONFIGURATIONS[:s3][:access_key_id].present?
  class ProActiveStorage::S3DirectUploadsControllerTest < ActionDispatch::IntegrationTest
    setup do
      @old_service = ProActiveStorage::Blob.service
      ProActiveStorage::Blob.service = ProActiveStorage::Service.configure(:s3, SERVICE_CONFIGURATIONS)
    end

    teardown do
      ProActiveStorage::Blob.service = @old_service
    end
    ...
    test "creating new direct upload" do
      ...
      response.parsed_body.tap do |details|
        assert_equal ProActiveStorage::Blob.find(details["id"]), ProActiveStorage::Blob.find_signed(details["signed_id"])
...
if SERVICE_CONFIGURATIONS[:gcs]
  class ProActiveStorage::GCSDirectUploadsControllerTest < ActionDispatch::IntegrationTest
    setup do
      @config = SERVICE_CONFIGURATIONS[:gcs]

      @old_service = ProActiveStorage::Blob.service
      ProActiveStorage::Blob.service = ProActiveStorage::Service.configure(:gcs, SERVICE_CONFIGURATIONS)
    end
...
if SERVICE_CONFIGURATIONS[:azure]
  class ProActiveStorage::AzureStorageDirectUploadsControllerTest < ActionDispatch::IntegrationTest
    setup do
      @config = SERVICE_CONFIGURATIONS[:azure]

      @old_service = ProActiveStorage::Blob.service
      ProActiveStorage::Blob.service = ProActiveStorage::Service.configure(:azure, SERVICE_CONFIGURATIONS)
    end
...
class ProActiveStorage::DiskDirectUploadsControllerTest < ActionDispatch::IntegrationTest
  test "creating new direct upload" do
    checksum = Digest::MD5.base64digest("Hello")
    ...
    @response.parsed_body.tap do |details|
      ...
      assert_equal checksum, details["checksum"]
      assert_equal "text/plain", details["content_type"]
      assert_match(/rails\/pro_active_storage\/disk/, details["direct_upload"]["url"])
