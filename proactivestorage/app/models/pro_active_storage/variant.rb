...
# gem).
#
#   Rails.application.config.pro_active_storage.variant_processor
#   # => :mini_magick
#
#   Rails.application.config.pro_active_storage.variant_processor = :vips
#   # => :vips
#
...
# ProActiveStorage::RepresentationsController.
#
# To refer to such a delayed on-demand variant, simply link to the variant through the resolved route provided
# by Pro Active Storage like so:
#
...
class ProActiveStorage::Variant
  ...
  # for a variant that points to the ProActiveStorage::RepresentationsController, which in turn will use this +service_call+ method
  # for its redirection.
  def service_url(expires_in: ProActiveStorage.service_urls_expire_in, disposition: :inline)
    service.url key, expires_in: expires_in, disposition: disposition, filename: filename, content_type: content_type
  end

  # Returns the receiving variant. Allows ProActiveStorage::Variant and ProActiveStorage::Preview instances to be used interchangeably.
  def image
    self
  end
  ...
  private
    ...
    def filename
      if WEB_IMAGE_CONTENT_TYPES.include?(blob.content_type)
        ...
        ProActiveStorage::Filename.new("#{blob.filename.base}.png")
      end
    end
