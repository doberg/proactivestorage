  ...
  class EngineTest < ActiveSupport::TestCase
    ...
    test "copying migrations" do
      ...
      Dir.chdir(app_path) do
        # Install Pro Active Storage migration file first so as not to affect test.
        `bundle exec rake pro_active_storage:install`
...
YAML
    ...
    test "pro_active_storage:install task works within engine" do
      @plugin.write "Rakefile", <<-RUBY
        APP_RAKEFILE = '#{app_path}/Rakefile'
        load 'rails/tasks/engine.rake'
      RUBY
      ...
      Dir.chdir(@plugin.path) do
        output = `bundle exec rake app:pro_active_storage:install`
        assert $?.success?, output

        pro_active_storage_migration = migrations.detect { |migration| migration.name == "CreateProActiveStorageTables" }
        assert pro_active_storage_migration
      end
    end
