...
### Pro Active Storage Setup

When working on Pro Active Storage, it is important to note that you need to
...
command inside of the `proactivestorage` directory to install the dependencies:

```bash
yarn install
```

Extracting previews, tested in ProActiveStorage's test suite requires third-party
applications, FFmpeg for video and muPDF for PDFs, and on macOS also XQuartz
and Poppler. Without these applications installed, ProActiveStorage tests will
raise errors.
