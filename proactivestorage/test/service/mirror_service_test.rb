...
class ProActiveStorage::Service::MirrorServiceTest < ActiveSupport::TestCase
  mirror_config = (1..3).map do |i|
    [ "mirror_#{i}",
      service: "Disk",
      root: Dir.mktmpdir("pro_active_storage_tests_mirror_#{i}") ]
  end.to_h

  config = mirror_config.merge \
    mirror:  { service: "Mirror", primary: "primary", mirrors: mirror_config.keys },
    primary: { service: "Disk", root: Dir.mktmpdir("pro_active_storage_tests_primary") }

  SERVICE = ProActiveStorage::Service.configure :mirror, config
  ...
  test "URL generation in primary service" do
    filename = ProActiveStorage::Filename.new("test.txt")
