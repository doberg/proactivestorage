...
require "pro_active_storage/downloader"
require "pro_active_storage/blob_key_gen"
...
class ProActiveStorage::Blob < ActiveRecord::Base
  require_dependency "pro_active_storage/blob/analyzable"
  require_dependency "pro_active_storage/blob/identifiable"
  require_dependency "pro_active_storage/blob/representable"

  include Analyzable
  include Identifiable
  include Representable

  self.table_name = "pro_active_storage_blobs"

  before_create { self.key = ProActiveStorage::BlobKeyGen.new(self).generate unless key? }

  store :metadata, accessors: [ :analyzed, :identified, :prefix ], coder: ActiveRecord::Coders::JSON

  class_attribute :service

  has_many :attachments

  scope :unattached, -> { left_joins(:attachments).where(ProActiveStorage::Attachment.table_name => { blob_id: nil }) }
  ...
  class << self
    ...
    def find_signed(id)
      find ProActiveStorage.verifier.verify(id, purpose: :blob_id)
    end
  ...
  # It uses the framework-wide verifier on <tt>ProActiveStorage.verifier</tt>, but with a dedicated purpose.
  def signed_id
    ProActiveStorage.verifier.generate(id, purpose: :blob_id)
  end
  ...
  def key
    ...
    self[:key] ||= ProActiveStorage::BlobKeyGen.new(self).generate
  end

  # Returns the format of the key that will be generated by ProActiveStorage::BlobKeyGen.
  def prefix
    super || ProActiveStorage.default_prefix
  end

  # Returns an ProActiveStorage::Filename instance of the filename that can be
  # queried for basename, extension, and a sanitized version of the filename
  # that's safe to use in URLs.
  def filename
    ProActiveStorage::Filename.new(self[:filename])
  end
  ...
  def service_url(expires_in: ProActiveStorage.service_urls_expire_in, disposition: :inline, filename: nil, **options)
    filename = ProActiveStorage::Filename.wrap(filename || self.filename)
  ...
  def service_url_for_direct_upload(expires_in: ProActiveStorage.service_urls_expire_in)
  ...
  #
  # The tempfile's name is prefixed with +ProActiveStorage-+ and the blob's ID. Its extension matches that of the blob.
  #
  ...
  #
  # Raises ProActiveStorage::IntegrityError if the downloaded data does not match the blob's checksum.
  def open(tempdir: nil, &block)
    ProActiveStorage::Downloader.new(self, tempdir: tempdir).download_blob_to_tempfile(&block)
  end
  ...
  # Enqueues an ProActiveStorage::PurgeJob job that'll call +purge+. This is the recommended way to purge blobs when the call
  # needs to be made from a transaction, a callback, or any other real-time scenario.
  def purge_later
    ProActiveStorage::PurgeJob.perform_later(self)
  end
  ...
  private
    ...
    def forcibly_serve_as_binary?
      ProActiveStorage.content_types_to_serve_as_binary.include?(content_type)
    end

    ActiveSupport.run_load_hooks(:pro_active_storage_blob, self)
end
