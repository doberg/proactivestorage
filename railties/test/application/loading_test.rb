...
class LoadingTest < ActiveSupport::TestCase
  ...
  test "descendants loaded after framework initialization are cleaned on each request without cache classes" do
    ...
    assert_equal [ProActiveStorage::Blob, ProActiveStorage::Attachment, ActiveRecord::SchemaMigration, ActiveRecord::InternalMetadata].collect(&:to_s).sort, ActiveRecord::Base.descendants.collect(&:to_s).sort
    get "/load"
    assert_equal [ProActiveStorage::Blob, ProActiveStorage::Attachment, ActiveRecord::SchemaMigration, ActiveRecord::InternalMetadata, Post].collect(&:to_s).sort, ActiveRecord::Base.descendants.collect(&:to_s).sort
    get "/unload"
    assert_equal [ProActiveStorage::Blob, ProActiveStorage::Attachment, ActiveRecord::SchemaMigration, ActiveRecord::InternalMetadata].collect(&:to_s).sort, ActiveRecord::Base.descendants.collect(&:to_s).sort
  end
